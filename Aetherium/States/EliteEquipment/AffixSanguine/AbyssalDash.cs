...
namespace Aetherium.States.EliteEquipment.AffixSanguine
{
    public class AbyssalDash : EntityState
    {
        ...
        private void CalculateBlinkDestination()
        {
            ...
            {
                nodes = isAerial ? SceneInfo.instance.groundNodes : SceneInfo.instance.airNodes;
            ...
            if(SanguineController && Vector3.Distance(SanguineController.LastPosition, blinkDestination) <= 2f)
            {
            ...
            var hurtBox = new RoR2.SphereSearch()
            {
                origin = blinkDestination,
                radius = characterBody.bestFitRadius,
                mask = LayerIndex.entityPrecise.mask,
            }.RefreshCandidates().FilterCandidatesByDistinctHurtBoxEntities().GetHurtBoxes();

            foreach (var hb in hurtBox)
            {
                if(hb.healthComponent && hb.healthComponent.body != characterBody)
                {
                    ModLogger.LogMessage($"Space occupied by {hb.healthComponent.body.name}");
                    GiveBackAIChaindashStackIfCancelled();
                    outer.SetNextStateToMain();
                    return;
                }
        ...
        public override void FixedUpdate()
        {
            ...
            if(characterBody.rigidbody)
            {
                ...
                characterBody.rigidbody.angularVelocity = Vector3.zero;
            }
